...
num_spk=2    # Number of speakers in the input audio
             # for EEND system, num_spk stands for the exact speaker number
             # for EESD system, num_spk stands for the maximum speaker number
             # more details in (https://ieeexplore.ieee.org/document/9747301)
...
# dnc diar model related
use_dnc=true # Whether to use EESD model with DNC label, default conduct vad + osd + diarization
vad_only=false     # whether to only conduct vad
osd_vad_only=false # whether to conduct vad + osd
...
Options:
    ...
    # DNC-based Diarization model specifics
    --use_dnc           # Whether to use EESD model with DNC label (default="${use_dnc}")
...
diar_stats_dir="${expdir}/diar_stats_${fs}_${train_set}"
...
if ! "${skip_data_prep}"; then
    ...
    if [ ${stage} -le 2 ] && [ ${stop_stage} -ge 2 ]; then
        ...
        for dset in ${test_sets}; do
            ...
            if "${use_dnc}"; then
                # further convert espnet_rttm into dnc_rttm
                pyscripts/utils/gen_dnc_rttm.py \
                    --rttm "${data_feats}${_suf}/${dset}"/espnet_rttm \
                    --output_path "${data_feats}${_suf}/${dset}" \
                    --vad_only ${vad_only} \
    ...
    if [ ${stage} -le 3 ] && [ ${stop_stage} -ge 3 ]; then
        ...
        # NOTE: Not applying to test_sets to keep original data
            ...
            if "${use_dnc}"; then
                # further convert espnet_rttm into dnc_rttm
                pyscripts/utils/gen_dnc_rttm.py \
                    --rttm "${data_feats}/${dset}"/espnet_rttm \
                    --output_path "${data_feats}/${dset}" \
                    --vad_only ${vad_only} \
                    --osd_vad_only ${osd_vad_only}
            fi
...
if ! "${skip_train}"; then
    if [ ${stage} -le 4 ] && [ ${stop_stage} -ge 4 ]; then
        ...
        # TODO(jiatong): add description for dnc_rttm and espnet_rttm
        if "${use_dnc}"; then
            rttm_type=dnc_rttm
            rttm_file=dnc_rttm
            if [ "${train_use_memory}" != "true" ]; then
                rttm_type=no_memory_dnc_rttm
            fi
        else
        ...
        ${train_cmd} JOB=1:"${_nj}" "${_logdir}"/stats.JOB.log \
            ${python} -m espnet2.bin.diar_train \
                ...
                --train_data_path_and_name_and_type "${_diar_train_dir}/${rttm_file},spk_labels,${rttm_type}" \
                --valid_data_path_and_name_and_type "${_diar_valid_dir}/${_scp},speech,${_type}" \
                --valid_data_path_and_name_and_type "${_diar_valid_dir}/${rttm_file},spk_labels,${rttm_type}" \
    ...
    if [ ${stage} -le 5 ] && [ ${stop_stage} -ge 5 ]; then
        ...
        _opts+="--train_data_path_and_name_and_type ${_diar_train_dir}/${rttm_file},spk_labels,${rttm_type} "
        ...
        _opts+="--valid_data_path_and_name_and_type ${_diar_valid_dir}/${rttm_file},spk_labels,${rttm_type} "
...
if ! "${skip_eval}"; then
    if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
        ...
        if "${use_dnc}"; then
            _opts+="--vad_only ${vad_only} "
            _opts+="--osd_vad_only ${osd_vad_only} "
            inference_binary=espnet2.bin.diar_dnc_inference
        else
            inference_binary=espnet2.bin.diar_dnc_inference
        fi
        ...
        for dset in ${test_sets}; do
            ...
            _dir="${diar_exp}/${inference_tag}_diarized_${dset}"
            _logdir="${_dir}/logdir"
            mkdir -p "${_logdir}"
    ...
    if [ ${stage} -le 7 ] && [ ${stop_stage} -ge 7 ]; then
        ...
        if "${vad_only}" || "${osd_vad_only}" ; then
            ...
            for dset in "${valid_set}" ${test_sets}; do
                _data="${data_feats}/${dset}"
                _inf_dir="${diar_exp}/${inference_tag}_diarized_${dset}"
                _dir="${diar_exp}/${inference_tag}_diarized_${dset}/vad_osd_scoring"
                mkdir -p "${_dir}"

                scripts/utils/score_vad_osd.sh \
                    --scoring_dir ${_dir} \
                    --infer_scp ${_inf_dir}/diarize.scp \
            ...
            for dset in "${valid_set}" ${test_sets}; do
                _data="${data_feats}/${dset}"
                _inf_dir="${diar_exp}/${inference_tag}_diarized_${dset}"
                _dir="${diar_exp}/${inference_tag}_diarized_${dset}/scoring"
                mkdir -p "${_dir}"
